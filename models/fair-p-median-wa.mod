...
minimize weighted_average_cost_dual_fairness:
        lambda * (sum {i in V, j in V} c[i,j]*x[i,j])
...
subject to weighted_average_area_constraint:
        lambda * (sum {i in V, j in V} c[i,j]*x[i,j]) + (1 - lambda) * (Dbeta*u + sum {i in V} v[i]) <= l;
