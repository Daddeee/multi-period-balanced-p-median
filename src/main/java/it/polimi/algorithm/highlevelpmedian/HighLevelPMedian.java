package it.polimi.algorithm.highlevelpmedian;
...
public class HighLevelPMedian {
    ...
    public int[] run() {
        ...
        try {
            ...
            ampl.read("models/high-level-p-median.mod");
            ampl.readData("");
            ampl.setOption("solver", "cplex");
            ampl.setOption("cplex_options", "threads=1");
            ampl.setOutputHandler(new OutputHandler() {
                @Override
                public void output(Kind kind, String s) {
                }
            });
           ...
           objective = ampl.getObjective("distance_and_displacement").value();
