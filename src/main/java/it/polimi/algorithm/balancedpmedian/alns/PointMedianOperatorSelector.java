...
public class PointMedianOperatorSelector extends OperatorsSelector {
    private static final Logger LOGGER = LoggerFactory.getLogger(PointMedianOperatorSelector.class);
    ...
    public PointMedianOperatorSelector(List<RuinOperator> ruinOperators, List<RecreateOperator> recreateOperators,
    ...
    public Solution search(Solution s) {
        ...
        if (d instanceof PointRemoval) {
            ...
            sol = (BPMSolution) r.recreate(d.ruin(s));
        ...
        LOGGER.info("#medians=" + sol.getMedianPoints().keySet().size() + " #points="
                + sol.getPointsMedian().keySet().size() + " #unassigned=" + sol.getUnassignedPointsTabu().keySet().size());
        return sol;
    }
