...
public class FairPMedianNadirExact implements Solver {

    private double beta;
    private double lambda;
    private double costbound;
    private double bmeanbound;

    private double cost;
    private double fairness;

    public FairPMedianNadirExact(double beta, double lambda, double costbound, double bmeanbound) {
        this.beta = beta;
        this.lambda = lambda;
        this.costbound = costbound;
        this.bmeanbound = bmeanbound;
    }
    ...
    public Solution run(Problem problem) {
        AMPL ampl = new AMPL();
        try {
            ampl.read("models/fair-p-median-nadir.mod");
            ampl.readData("");
            ...
            Parameter costbound = ampl.getParameter("costbound");
            costbound.setValues(this.costbound);

            Parameter bmeanbound = ampl.getParameter("bmeanbound");
            bmeanbound.setValues(this.bmeanbound);
