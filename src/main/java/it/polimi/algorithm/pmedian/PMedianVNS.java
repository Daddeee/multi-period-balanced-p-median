...
public class PMedianVNS implements Solver {
    ...
    public PMedianVNS(int seed) {
        this(new Random(seed));
    }

    public PMedianVNS(Random random) {
        this.random = random;
    }
    ...
    public Solution run(Problem prob) {
        ...
        while (k < kmax) {
            ...
            // Local search
            fcur = fastInterchange(xcur, xidx, c1cur, c2cur, fcur, n, p, d);
    ...
    private float fastInterchange(int[] xopt, int[] xidx, int[] c1, int[] c2, float fopt, int n, int p, float[][] d) {
        while(true) {
            ...
            for (int i=p; i < n; i++) {
                int goin = xopt[i];
                Pair<Integer, Float> pair = move(xopt, xidx, c1, c2, goin, n, p, d);
                int goout = pair.getFirst();
                float w = pair.getSecond();
                if (w < wopt) {
                    wopt = w;
            ...
            update(xopt, c1, c2, goinopt, gooutopt, n, p, d);
        }
    }
}
