...
public class FairPMedianExactTest {
    @Test
    public void augerat() {
        String basePath = "instances/augerat/A-VRP/";
        String baseZonePath = "instances/augerat/zones/fair/";
        ...
        for (int i=0; i<files.length; i++) {
            ...
            FairPMedianExact exact = new FairPMedianExact(0.4, 0.5);
            ...
            ZonesCSVWriter.write("results/fairpmedian/fpmp-" + files[i],
                    problem.getServices().stream().map(Service::getLocation).toArray(Location[]::new), exactSolution.getMedians());
        }

        TestCSVWriter.write("results/fairpmedian/augerat.csv", opt, opttimes, costs, avgs, bmeans, n, p);
    }
